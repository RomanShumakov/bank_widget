...
@patch("requests.get")
def test_transaction_in_rub_uncorrect_payload_data(mock_get: Mock) -> None:
    """Функция проверки работоспособности функции при неправильных данных, полученных при HTTP-запросе"""
    mock_get.return_value.json.return_value = {}
    oper = {
        "id": 41428829,
        "state": "EXECUTED",
...
@patch("requests.get")
def test_transaction_in_rub_uncorrect_income_data(mock_get: Mock) -> None:
    """Функция проверки работоспособности функции при неправильных входных данных (пустым словарем)"""
    mock_get.return_value.json.return_value = {
        "success": True,
        "query": {"from": "USD", "to": "RUB", "amount": 8221.37},
        "info": {"timestamp": 1779613387, "rate": 71.790516},
